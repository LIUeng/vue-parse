...
- 触发的条件
  - activeEffect 记录当前初始化 ReactiveEffect 实例化的结果，等待处理更新
- 渲染函数
  - trackEffect 订阅触发
  - triggerEffects
    - \_dirtyLevel 函数参数
...
- 接收 getter/setter 函数参数
  - 实例化 new ComputedRefImpl，创建 .value 值
    - 实例化 effect = ReactiveEffect getter 函数，等待执行更新
  - 更新过程
    - set value 记录发布者列表，等待更新
      - triggerRefValue/triggerEffects 触发响应式数据
    - get value 获取响应式数据发布者
      - 添加发布者 dep
      - trackRefValue/trackEffect 追踪响应式数据
      - 触发 getter 函数执行，获取新值
...
(function (exports, factory) {
  ...
  // 渲染后，保证 trackId _depsLength 一致性
  ...
  // 记录当前响应式数据 ref 引用值 new Proxy 监听过的缓存
